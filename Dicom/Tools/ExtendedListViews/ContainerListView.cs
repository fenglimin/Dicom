...
namespace SynapticEffect.Forms
{
	...
	public class ContainerListView: Control
	{
		...
		protected string TruncatedString(string s, int width, int offset, Graphics g)
		{
			...
			try
			{
				if (s.Length > 1024) s = s.Substring(0, 1023);
		...
		protected virtual void DrawHeaders(Graphics g, Rectangle r)
		{
			...
			if (headerStyle != ColumnHeaderStyle.None)
			{
				g.FillRectangle(new SolidBrush(Color.Red), r.Left+2, r.Top+2, r.Width-2, headerBuffer);
				...
				for (i=0; i<columns.Count; i++)
				{
					...
					{						
						...
						//else
						//	System.Windows.Forms.ControlPaint.DrawButton(g, lp_scr+last, r.Top+2, columns[i].Width, r.Top+headerBuffer, ButtonState.Pushed);

						g.DrawRectangle(new Pen(SystemBrushes.Control), lp_scr + last, r.Top + 2, columns[i].Width, r.Top + headerBuffer);
						g.FillRectangle(new SolidBrush(Color.WhiteSmoke), lp_scr + last+1, r.Top + 3, columns[i].Width-2, r.Top + headerBuffer-2);
						...
						else
						{
							...
							if (columns[i].TextAlign == HorizontalAlignment.Left)
								g.DrawString(TruncatedString(columns[i].Text, columns[i].Width, 0, g), font, SystemBrushes.ControlText, (float)(lp_scr+last+4), (float)(r.Top+5));
				...
				{
					//g.Clip = new Region(new Rectangle(r.Left+2, r.Top+2, r.Width-5, r.Top+headerBuffer));
					var rect = new Rectangle(lp_scr + last + 2, r.Top + 2, r.Width - 5, r.Top + headerBuffer);
					g.DrawRectangle(new Pen(SystemBrushes.Control), rect.Left, rect.Top, rect.Width, rect.Height);
					g.FillRectangle(new SolidBrush(Color.WhiteSmoke), rect.Left+1, rect.Top+1, rect.Width-2, rect.Height-2);
					...
					//g.DrawString("", this.Font, SystemBrushes.ControlText, (float)(lp_scr + last + 4), (float)(r.Top + 5));
				}
			}
		}		
